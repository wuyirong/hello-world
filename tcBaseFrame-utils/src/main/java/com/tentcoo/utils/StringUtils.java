...
//	}
	...
	public static boolean email(String str) {
		...
		boolean b = false;
		p = Pattern
				.compile("^([a-zA-Z0-9_\\-\\.]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([a-zA-Z0-9\\-]+\\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$"); // 验证手机号
		m = p.matcher(str);
		b = m.matches();
		return b;
	}
	...
	public static String getRandomStrFromArray(String[] arras){
		if(arras==null){ return null; }
		//先随机产生一个下标再获取元素
		String random = "";
		int index = (int)(Math.random() * arras.length);
		random = arras[index];
		return random;
	}
